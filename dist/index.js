...
var ENV = {
  ...
  publicApiKeys: (process.env.VEYRITH_API_KEYS ?? process.env.VEYRITH_API_KEY ?? "").split(",").map((value) => value.trim()).filter(Boolean),
...
async function recordApiUsage(record) {
  const db = await getDb();
  if (!db) return;
  await db.insert(apiUsage).values(record);
}
async function getApiUsageSummary(since) {
  const db = await getDb();
  if (!db)
    return {
      totalRequests: 0,
  ...
  const [summary] = await db.select({
    ...
    successfulRequests: sql`sum(case when ${apiUsage.statusCode} < 400 then 1 else 0 end)`,
    generatedRequests: sql`sum(case when ${apiUsage.generated} = 'yes' then 1 else 0 end)`,
    averageLatencyMs: sql`coalesce(avg(${apiUsage.latencyMs}), 0)`
  }).from(apiUsage).where(gte(apiUsage.createdAt, since));
  const latest = await db.select().from(apiUsage).where(gte(apiUsage.createdAt, since)).orderBy(desc(apiUsage.createdAt)).limit(25);
  return {
...
var blueprintInputSchema = z2.object({
  brief: z2.string().min(20).max(5e3)
});
var blueprintSchema = z2.object({
  title: z2.string(),
  summary: z2.string(),
  confidence: z2.number().min(0).max(100),
  architectureStyle: z2.string(),
  assumptions: z2.array(z2.string()).min(2).max(5),
  components: z2.array(
    z2.object({
      name: z2.string(),
      type: z2.string(),
      description: z2.string(),
      health: z2.enum(["strong", "watch", "risk"])
    })
  ).min(4).max(8),
  risks: z2.array(
    z2.object({
...
var fallbackBlueprint = {
  ...
  summary: "A resilient event-driven core with a calm read path: accept payments once, fan out safely, and keep customer-facing queries fast even when providers wobble.",
  confidence: 82,
  ...
  nextMove: "Lock the payment state machine and write the idempotency contract before choosing infrastructure."
};
function messageText(content) {
  if (typeof content === "string") return content;
  if (Array.isArray(content)) {
    return content.map(
      (part) => typeof part === "object" && part && "text" in part ? String(part.text ?? "") : ""
    ).join("");
  }
  return "";
}
...
async function generateBlueprint(brief) {
  try {
    const { data } = await listLLMModels();
    const preferred = data.find((model) => model.id === "claude-sonnet-4-6")?.id ?? data.find((model) => model.id === "gpt-5-mini")?.id;
    const response = await invokeLLM({
      model: preferred,
      messages: [
        {
          role: "system",
          content: "You are Veyrith, a principal software architect who thinks in failure modes, contracts, and deliberate tradeoffs. Generate an honest, practical first-pass architecture blueprint from the user's product brief. Prefer one coherent architecture over a shopping list. Make assumptions explicit. Output only JSON that matches the requested schema."
        },
        { role: "user", content: `Product brief:
${brief}` }
      ],
      thinking: preferred?.startsWith("claude-") ? { type: "enabled", budget_tokens: 1536 } : void 0,
      reasoning: preferred?.startsWith("gpt-") ? { effort: "low" } : void 0,
      response_format: {
        type: "json_schema",
        json_schema: {
...
var appRouter = router({
  ...
  blueprint: router({
    generate: publicProcedure.input(blueprintInputSchema).mutation(async ({ input }) => generateBlueprint(input.brief))
  })
});
...
function getToken(req) {
  ...
  if (extra || scheme?.toLowerCase() !== "bearer" || !token) return void 0;
  return token;
}
...
function consumeRateLimit(fingerprint) {
  const now = Date.now();
  ...
  return {
    ...
    retryAfter: Math.max(
      1,
      Math.ceil((windowMs - (now - current.startedAt)) / 1e3)
    )
  };
}
...
async function logUsage(endpoint, statusCode, latencyMs, generated, keyFingerprint) {
  try {
...
function registerPublicApi(app) {
  app.post("/api/v1/blueprints", async (req, res) => {
    ...
    return res.status(200).json({ data: blueprint, meta: { apiVersion: "v1", advisory: true } });
  });
  app.get("/api/v1/usage", async (req, res) => {
    ...
    return res.status(200).json({ data: summary, meta: { apiVersion: "v1", window: "24h" } });
  });
}
